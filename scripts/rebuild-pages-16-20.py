#!/usr/bin/env python3
"""
Rebuild Shell Scripting Handbook carousel pages 16-20.

Uses:
- Colorful filled SVG icons from scripts/shell_scripting_icons.py
- Canonical notebook CSS at /uploads/notebook-template/notebook-template.css
- VLM analysis for each page at /tmp/analysis-page-NN.json

Layouts:
- Page 16 (Networking): 3-column card grid
- Page 17 (Error Handling): 2-column (rows + sidebar)
- Page 18 (Cron): 2-column (rows + sidebar)
- Page 19 (Debugging): 2-column (rows + sidebar)
- Page 20 (DevOps): 2-column (rows + sidebar)
...
# Import the colorful filled SVG icon library
...
CANONICAL_CSS_FILE = PROJECT_ROOT / "public" / "uploads" / "notebook-template" / "notebook-template.css"
CANONICAL_CSS_PATH = "/uploads/notebook-template/notebook-template.css"  # for HTTP serving

# Inline canonical CSS so each page is self-contained (renders via file:// too)
try:
    CANONICAL_CSS_INLINE = CANONICAL_CSS_FILE.read_text(encoding="utf-8")
except FileNotFoundError:
    CANONICAL_CSS_INLINE = ""
...
# ============================================================
# SUPPLEMENTAL CSS (in addition to canonical notebook CSS)
# ============================================================
SUPPLEMENTAL_CSS = """
/* ============================================================
...
.page-title .num{color:var(--green);}
.page-title .title-ico{display:inline-flex;width:42px;height:42px;}
.page-subtitle{
...
.page-badge{
  position:absolute;top:32px;right:50px;z-index:3;
...
/* ============================================================
   ...
   ============================================================ */
.terminal{
  background:var(--term-bg);border-radius:8px;overflow:hidden;
...
.terminal .term-body .prompt{color:var(--terminal-green,#7ee787);}
.terminal .term-body .comment{color:var(--terminal-dim,#8b949e);font-style:italic;}
...
.callout .ico{flex-shrink:0;width:20px;height:20px;margin-top:1px;display:inline-flex;}
.callout .ico svg{display:block;}

/* ============================================================
   3-COLUMN CARD GRID (page 16)
   ============================================================ */
.card-grid-3{
  display:grid;
...
.grid-card .gc-body .terminal .term-body{font-size:10.5px;padding:8px 12px;}

/* Color variants for grid-card heads */
...
.compare .col h4 .h-ico{width:16px;height:16px;display:inline-flex;}
.compare .col h4 .h-ico svg{display:block;}
.compare .col p{margin-bottom:4px;color:var(--ink-2);}
.compare .col .fix{
...
.compare .col .mini-code{
  ...
  padding:5px 7px;border-radius:4px;margin-top:4px;
  white-space:pre-wrap;
}
.compare-grid-2{
  display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:8px;
}
.compare-grid-2 .compare{grid-template-columns:1fr;}
...
.workflow .wf-step .wf-ico{width:32px;height:32px;display:inline-flex;}
.workflow .wf-step .wf-ico svg{display:block;}
.workflow .wf-arrow{color:var(--green);font-weight:800;font-size:16px;}
.workflow .wf-final{
...
/* ============================================================
   ...
   ============================================================ */
.page-foot-note{
  font-size:10.5px;color:var(--ink-2);text-align:center;
  font-style:italic;margin-top:8px;opacity:.8;
}
"""
...
def terminal(code: str, title: str = "bash") -> str:
    ...
    return f"""<div class="terminal"><div class="term-bar"><span class="term-dot dot-red"></span><span class="term-dot dot-amber"></span><span class="term-dot dot-green"></span><span class="term-title">{title}</span></div><div class="term-body">{code}</div></div>"""


def callout(text: str, style: str = "tip", ico_name: str = None) -> str:
    """Build a callout box."""
    ico_html = ""
    if ico_name:
        ico_html = f'<span class="ico">{icon(ico_name, 20)}</span>'
    return f'<div class="callout {style}">{ico_html}<div>{text}</div></div>'


def row(icon_name: str, num: str, heading: str, desc: str = "",
        inner_html: str = "") -> str:
    """Build a row with colorful icon, heading, and inner content."""
    desc_html = f"<p>{desc}</p>" if desc else ""
    return f"""<div class="row"><div class="ico">{icon(icon_name, 36)}</div><div class="text"><h3>{num}{heading}</h3>{desc_html}{inner_html}</div></div>"""


def sidebar_card(icon_name: str, heading: str, body_html: str) -> str:
    """Build a sidebar card."""
    return f"""<div class="card"><div class="head"><span class="head-ico">{icon(icon_name, 18)}</span><span>{heading}</span></div><div class="body-pad">{body_html}</div></div>"""


def grid_card(icon_name: str, heading: str, color: str, body_html: str) -> str:
    """Build a colorful grid card (3-column layout)."""
    return f"""<div class="grid-card"><div class="gc-head gc-{color}"><span class="gc-ico">{icon(icon_name, 18)}</span><span>{heading}</span></div><div class="gc-body">{body_html}</div></div>"""


def key_takeaway(text: str, ico_name: str = "lightbulb",
                 heading: str = "KEY TAKEAWAY") -> str:
    """Build a key takeaway box."""
    return f"""<div class="key-takeaway"><span class="kt-ico">{icon(ico_name, 32)}</span><div class="kt-text"><strong>{heading}</strong>{text}</div></div>"""


def page_skeleton(title_num: str, title: str, subtitle: str,
                  page_num: int, body_html: str) -> str:
    """Wrap body content in the full HTML page skeleton."""
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>{title_num}. {title} — Shell Scripting for DevOps</title>
<style>
/* ===== CANONICAL NOTEBOOK TEMPLATE CSS (inlined for portability) ===== */
{CANONICAL_CSS_INLINE}

/* ===== SUPPLEMENTAL CSS (page-specific layouts) ===== */
{SUPPLEMENTAL_CSS}
</style>
</head>
...
<div class="page-wrapper">
  ...
  <div class="page">
    ...
    <div class="page-badge">Page {page_num} of 20</div>
    <div class="content">

      <h1 class="page-title"><span class="title-ico">{icon('shell', 38)}</span><span class="num">{title_num}.</span> {title}</h1>
...
# ============================================================
# PAGE 16 — NETWORKING WITH SHELL SCRIPTS (3-column grid)
# ============================================================
def build_page_16() -> str:
    """Page 16: 3-column card grid for networking commands."""
    cards_html = f"""
      <div class="card-grid-3">
        {grid_card("globe", "1. CURL — HTTP REQUESTS", "green", """
          ...
          <ul><li>Fetch URLs</li><li>Test APIs</li><li>Check headers</li></ul>
        """)}
        {grid_card("monitor", "2. PING — CONNECTIVITY", "blue", """
          ...
          <ul><li>Check reachability</li><li>Measure latency</li><li>Troubleshoot</li></ul>
        """)}
        {grid_card("network", "3. SS — CHECK SOCKETS", "purple", """
          <p>Show network sockets and connections.</p>
          {terminal('<span class="prompt">$</span> <span class="cmd">ss</span> -tuln  <span class="comment"># listening</span>\\n<span class="prompt">$</span> <span class="cmd">ss</span> -tnp   <span class="comment"># connections</span>\\n<span class="prompt">$</span> <span class="cmd">ss</span> -ltnp  <span class="comment"># w/ PID</span>', 'ss')}
          <ul><li>Listening ports</li><li>Active connections</li><li>PIDs</li></ul>
        """)}
        {grid_card("server", "4. HOSTNAME — SYSTEM NAME", "gold", """
          ...
          <ul><li>Identify hosts</li><li>Find IPs</li><li>Audit fleet</li></ul>
        """)}
        {grid_card("globe", "5. DNS CHECKS — RESOLVE", "teal", """
          ...
          <ul><li>Resolve names</li><li>Inspect records</li><li>Debug DNS</li></ul>
        """)}
        {grid_card("warning", "6. HTTP STATUS CODES", "red", """
          ...
          <p style="margin-top:6px;font-size:10.5px;">Use <code class="inline">curl -w "%{http_code}"</code> to capture status.</p>
        """)}
        {grid_card("check-circle", "7. TEST ENDPOINT UP", "green", """
          <p>Quickly test if an endpoint is up.</p>
          {terminal('<span class="prompt">$</span> <span class="cmd">curl</span> -s -o /dev/null -w <span class="str">"%{{http_code}}"</span> URL\\n<span class="comment"># 200-399 = UP</span>\\n<span class="comment"># 400-599 = DOWN</span>\\n<span class="comment"># 000 = no response</span>', 'test')}
          <ul><li>Verify uptime</li><li>Detect outages</li><li>Alert teams</li></ul>
        """)}
        {grid_card("bell", "8. APP HEALTH CHECK", "blue", """
          <p>Check app health endpoint.</p>
          {terminal('<span class="prompt">$</span> <span class="cmd">curl</span> -s <span class="str">https://app/health</span>\\n<span class="prompt">$</span> <span class="cmd">curl</span> -sf URL <span class="kw">&amp;&amp;</span> <span class="cmd">echo</span> OK\\n  <span class="kw">||</span> <span class="cmd">echo</span> DOWN', 'health')}
          <ul><li>Expose /health</li><li>Monitor apps</li><li>Catch issues early</li></ul>
        """)}
        {grid_card("shield", "9. WEBSITE SCRIPT", "purple", """
          <p>Sample availability script.</p>
          {terminal('<span class="comment">#!/bin/bash</span>\\n<span class="cmd">URL</span>=<span class="str">"https://example.com"</span>\\n<span class="cmd">STATUS</span>=$(<span class="cmd">curl</span> -so /dev/null -w "%{{http_code}}" "$URL")\\n<span class="kw">if</span> [ "$STATUS" = "200" ]; <span class="kw">then</span>\\n  <span class="cmd">echo</span> "UP"\\n<span class="kw">else</span>\\n  <span class="cmd">echo</span> "DOWN"\\n<span class="kw">fi</span>', 'avail.sh')}
        """)}
      </div>

      <div class="card-grid-3">
        {grid_card("list", "10. QUICK REFERENCE", "navy", """
          <p>Common networking commands.</p>
          <ul>
            <li><strong>Test:</strong> ping, curl, nc, telnet</li>
            <li><strong>Monitor:</strong> ss, netstat, iftop</li>
            <li><strong>Ensure:</strong> /health endpoint</li>
            <li><strong>Alert:</strong> on 5xx or timeouts</li>
            <li><strong>Log:</strong> response codes & times</li>
          </ul>
        """)}
        {grid_card("check-circle", "11. BEST PRACTICES", "green", """
          <p>Network script best practices.</p>
          <ul>
            <li>Check status codes, not just ping</li>
            <li>Set timeouts on every curl</li>
            <li>Log every check to file</li>
            <li>Use retries for flaky links</li>
            <li>Validate DNS before assuming down</li>
            <li>Alert only after N failures</li>
            <li>Use full URLs with protocol</li>
          </ul>
        """)}
        {grid_card("lightbulb", "12. KEY TAKEAWAY", "gold", """
          <p><strong>Check network.</strong> Verify health. Ensure uptime.</p>
          <p style="margin-top:6px;font-size:10.5px;">Network scripts keep services reliable. Always test, monitor, and alert.</p>
        """)}
      </div>

      {callout('<strong>Pro tip:</strong> Combine <code class="inline">curl</code>, <code class="inline">ping</code>, and <code class="inline">ss</code> in one script to fully diagnose any connectivity issue.', 'tip', 'lightbulb')}
    """
    ...
    body_html = f"""
      <div class="body single">
        <div class="rows">
          <div class="row">
            <div class="ico">{icon('globe', 36)}</div>
            <div class="text">
              <h3>NETWORKING WITH SHELL SCRIPTS — TOOLKIT</h3>
              <p>Test connectivity, monitor services, and ensure availability with these essential commands.</p>
            </div>
          </div>
        </div>
      </div>
...
# ============================================================
# PAGE 17 — ERROR HANDLING & SAFER SCRIPTS (2-column)
# ============================================================
def build_page_17() -> str:
    """Page 17: 2-column rows + sidebar for safer shell scripts."""

    rows_html = f"""
      {row("warning", "1. ", "WHY SCRIPTS FAIL SILENTLY",
          "By default, bash ignores errors and keeps going — hiding real issues.",
          callout('<strong>Problem:</strong> A failed command does not stop the script. The next line runs anyway, often producing wrong results.', 'danger', 'bug'))}

      {row("shield", "2. ", "SET -E: EXIT ON ERROR",
          "Stop the script as soon as any command fails.",
          terminal('<span class="cmd">set</span> -e\\n<span class="comment"># script exits if any command returns non-zero</span>') +
          callout('<strong>Benefit:</strong> Catches the first failure immediately.', 'tip', 'check-circle'))}

      {row("user", "3. ", "SET -U: ERROR ON UNSET VARS",
          "Fail when an undefined variable is used.",
          terminal('<span class="cmd">set</span> -u\\n<span class="cmd">echo</span> <span class="str">"$UNDEFINED"</span>  <span class="comment"># fails fast</span>') +
          callout('<strong>Benefit:</strong> Catches typos and missing inputs.', 'tip', 'check-circle'))}

      {row("code", "4. ", "SET -O PIPEFAIL: CATCH PIPE ERRORS",
          "Detect failures inside pipelines.",
          terminal('<span class="cmd">set</span> -o pipefail\\n<span class="cmd">cat</span> file.txt <span class="kw">|</span> <span class="cmd">grep</span> <span class="str">"ERROR"</span> <span class="kw">|</span> <span class="cmd">wc</span> -l') +
          callout('<strong>Benefit:</strong> Without pipefail, only the last command\'s status is checked.', 'tip', 'check-circle'))}

      {row("shield", "5. ", "SET -EUO PIPEFAIL: STRONG &amp; SAFE MODE",
          "Best practice — combine all three flags.",
          terminal('<span class="cmd">set</span> -euo pipefail') +
          '<ul><li>Exit on any command error</li><li>Error on unset variables</li><li>Catch pipeline failures</li></ul>')}

      {row("clipboard", "6. ", "VALIDATING INPUTS",
          "Always verify inputs before using them.",
          ...
          callout('<strong>Benefit:</strong> Prevents errors and makes scripts user-friendly.', 'tip', 'check-circle'))}

      {row("gear", "7. ", "CHECKING DEPENDENCIES",
      ...
      {row("bug", "8. ", "HANDLING FAILED COMMANDS",
          ...
          callout('<strong>Benefit:</strong> React to failures and prevent bad outcomes.', 'tip', 'check-circle'))}

      {row("refresh", "9. ", "CLEANING UP AFTER FAILURES",
          "Use traps to clean up on exit or error.",
      ...
      {row("target", "10. ", "WRITING PREDICTABLE AUTOMATION",
          "Follow these rules for reliable scripts:",
          '<ul><li>Fail fast</li><li>Validate inputs</li><li>Check dependencies</li><li>Handle errors</li><li>Clean up properly</li><li>Log what happens</li><li>Exit with the right status code</li></ul>')}

      {row("code", "EXAMPLE: ", "SAFE SCRIPT TEMPLATE",
          "A complete template combining all safety flags.",
          terminal('<span class="comment">#!/bin/bash</span>\\n<span class="cmd">set</span> -euo pipefail\\n\\n<span class="comment"># Check input</span>\\n<span class="kw">if</span> [ -z <span class="str">"$1"</span> ]; <span class="kw">then</span>\\n  <span class="cmd">echo</span> <span class="str">"Usage: $0 &lt;url&gt;"</span>; <span class="cmd">exit</span> 1\\n<span class="kw">fi</span>\\n\\n<span class="comment"># Check dependency</span>\\n<span class="cmd">command</span> -v curl <span class="kw">&gt;/dev/null</span> 2&gt;&amp;1 <span class="kw">||</span> {{ <span class="cmd">echo</span> <span class="str">"curl not found"</span>; <span class="cmd">exit</span> 1; }}\\n\\n<span class="comment"># Cleanup trap</span>\\n<span class="cmd">cleanup</span>() {{ <span class="cmd">rm</span> -f temp.log; }}\\n<span class="cmd">trap</span> cleanup EXIT\\n\\n<span class="comment"># Main logic</span>\\n<span class="cmd">URL</span>=<span class="str">"$1"</span>\\n<span class="cmd">HTTP_CODE</span>=$(<span class="cmd">curl</span> -s -o /dev/null -w <span class="str">"%{{http_code}}"</span> <span class="str">"$URL"</span>)\\n<span class="kw">if</span> [ <span class="str">"$HTTP_CODE"</span> -ge 200 ] <span class="kw">&amp;&amp;</span> [ <span class="str">"$HTTP_CODE"</span> -lt 400 ]; <span class="kw">then</span>\\n  <span class="cmd">echo</span> <span class="str">"[$URL] UP (HTTP $HTTP_CODE)"</span>\\n<span class="kw">else</span>\\n  <span class="cmd">echo</span> <span class="str">"[$URL] DOWN (HTTP $HTTP_CODE)"</span>; <span class="cmd">exit</span> 1\\n<span class="kw">fi</span>', 'safe.sh'))}
    """

    sidebar_html = f"""
      {sidebar_card("warning", "WHY SCRIPTS FAIL", """
        <p>Without safety flags, scripts:</p>
        <ul>
          <li>Ignore command failures</li>
          <li>Use unset variables as empty</li>
          <li>Hide pipeline errors</li>
          <li>Continue after critical issues</li>
        </ul>
      """)}

      {sidebar_card("shield", "SAFE-MODE FLAGS", """
        <p>The four essential <code class="inline">set</code> options:</p>
        <table class="tbl">
          <thead><tr><th>Flag</th><th>Effect</th></tr></thead>
          <tbody>
            <tr><td><code>-e</code></td><td>Exit on error</td></tr>
            <tr><td><code>-u</code></td><td>Error on unset var</td></tr>
            <tr><td><code>-o pipefail</code></td><td>Catch pipe errors</td></tr>
            <tr><td><code>-euo pipefail</code></td><td>All combined</td></tr>
          </tbody>
        </table>
      """)}

      {sidebar_card("check-circle", "PREDICTABLE AUTOMATION", """
        <p>Rules for reliable scripts:</p>
        <ul>
          <li>Fail fast</li>
    ...
    return page_skeleton(
        ...
        subtitle="FAIL FAST. VALIDATE. CLEAN UP. STAY PREDICTABLE.",
        page_num=17,
        body_html=body_html,
    )
...
# ============================================================
# PAGE 18 — SCHEDULING SCRIPTS WITH CRON (2-column)
# ============================================================
def build_page_18() -> str:
    """Page 18: 2-column rows + sidebar for cron scheduling."""
    ...
    rows_html = f"""
      ...
      {row("list", "3. ", "CRON TIMING SYNTAX",
          ...
          '<table class="tbl"><thead><tr><th>Field</th><th>Range</th></tr></thead><tbody><tr><td>MIN</td><td>0–59</td></tr><tr><td>HOUR</td><td>0–23</td></tr><tr><td>DAY</td><td>1–31</td></tr><tr><td>MONTH</td><td>1–12</td></tr><tr><td>DOW</td><td>0–6 (0=Sun)</td></tr></tbody></table>' +
          '<p style="font-size:11px;margin-top:6px;font-weight:800;color:var(--green);">EXAMPLES</p>' +
          '<table class="tbl"><thead><tr><th>Pattern</th><th>Meaning</th></tr></thead><tbody><tr><td>* * * * *</td><td>Every minute</td></tr><tr><td>0 * * * *</td><td>Every hour at :00</td></tr><tr><td>0 2 * * *</td><td>Daily at 2 AM</td></tr><tr><td>30 1 * * 1</td><td>Mon at 1:30 AM</td></tr><tr><td>0 3 1 * *</td><td>1st of month at 3 AM</td></tr></tbody></table>')}
      ...
      {row("monitor", "5. ", "ENVIRONMENT DIFFERENCES",
          "Cron runs with a minimal environment. Your PATH, variables, and shell may be different. Use full paths in scripts. Set needed variables inside the script.",
      ...
      {row("file", "6. ", "LOGGING CRON OUTPUT",
          ...
          terminal('<span class="comment">*/5 * * * * /path/to/script.sh \\</span>\\n  <span class="kw">&gt;&gt;</span> /var/log/mycron.log 2&gt;&amp;1'))}

      {row("warning", "7. ", "COMMON CRON FAILURES",
          "Watch for these pitfalls:",
          '<ul><li>Wrong timing syntax</li><li>Script path incorrect</li><li>Script not executable</li><li>Missing environment variables</li><li>No permissions to run script</li><li>Output not logged</li><li>Cron service not running</li></ul>')}

      {row("refresh", "8. ", "SCHEDULING BACKUP &amp; CLEANUP",
          "Two practical scheduled-job examples:",
          terminal('<span class="comment"># BACKUP JOB — daily at 2 AM</span>\\n0 2 * * * /backup.sh \\<span class="kw">&gt;&gt;</span> /var/log/backup.log 2&gt;&amp;1\\n\\n<span class="comment"># CLEANUP JOB — weekly on Sunday at 3:30 AM</span>\\n30 3 * * 0 /cleanup.sh \\<span class="kw">&gt;&gt;</span> /var/log/cleanup.log 2&gt;&amp;1', 'crontab'))}
    """
...
# ============================================================
# PAGE 19 — DEBUGGING SHELL SCRIPTS (2-column)
# ============================================================
def build_page_19() -> str:
    """Page 19: 2-column rows + sidebar for debugging."""
...
./script.sh: line 5: NAME:
  ...
  No such file or directory</div>
    ...
    # Replace placeholders with colorful icons
    compare_cards = compare_cards.replace("__BUG_A__", icon("bug", 16))
    compare_cards = compare_cards.replace("__BUG_B__", icon("bug", 16))
    compare_cards = compare_cards.replace("__BUG_C__", icon("bug", 16))
    compare_cards = compare_cards.replace("__BUG_D__", icon("bug", 16))
    compare_cards = compare_cards.replace("__BUG_E__", icon("bug", 16))
    compare_cards = compare_cards.replace("__BUG_F__", icon("bug", 16))
...
# ============================================================
# PAGE 20 — SHELL SCRIPTING IN DEVOPS WORKFLOWS (2-column)
# ============================================================
def build_page_20() -> str:
    """Page 20: 2-column rows + sidebar for DevOps workflows."""

    rows_html = f"""
      {row("refresh", "1. ", "SHELL SCRIPTS IN CI/CD",
          "Automate build, test, and deploy across every stage.",
          '<ul><li>Automate build, test, and deploy</li><li>Run commands at every stage</li><li>Make pipelines reliable and repeatable</li><li>Handle setup, checks, and notifications</li></ul>')}

      {row("branch", "2. ", "GITHUB ACTIONS",
          "Use shell scripts in workflow steps.",
          '<ul><li>Run custom commands easily</li><li>Prepare environments</li><li>Build, test, and deploy apps</li></ul>' +
      ...
      {row("server", "3. ", "JENKINS",
          ...
          '<ul><li>Automate jobs and deployments</li><li>Manage environments</li><li>Integrate with other tools</li></ul>' +
          terminal('<span class="comment">#!/bin/bash</span>\\n./build.sh <span class="kw">&amp;&amp;</span> ./deploy.sh', 'jenkins'))}

      {row("container", "4. ", "DOCKER",
          "Build and push images with shell scripts.",
          '<ul><li>Build and push images</li><li>Prepare container environments</li><li>Clean up unused resources</li><li>Manage volumes and networks</li></ul>' +
      ...
      {row("network", "5. ", "KUBERNETES",
          ...
          '<ul><li>Deploy and update applications</li><li>Manage configs and secrets</li><li>Check pod and service status</li></ul>' +
      ...
      {row("cloud", "6. ", "CLOUD VM INITIALIZATION",
          ...
          '<ul><li>Run scripts on first boot</li><li>Install packages and tools</li><li>Configure users and services</li><li>Set up security and updates</li></ul>' +
      ...
      {row("rocket", "7. ", "DEPLOYMENT SCRIPTS",
          ...
          '<ul><li>Automate deployments</li><li>Backup before updates</li><li>Rollback on failure</li><li>Zero- or low-downtime</li></ul>' +
          terminal('./deploy.sh --env prod\\n./rollback.sh', 'deploy'))}

      {row("bell", "8. ", "HEALTH CHECKS",
          "Verify apps and services stay up.",
          '<ul><li>Check app and services</li><li>Validate endpoints</li><li>Alert on failure</li><li>Keep systems reliable</li></ul>' +
      ...
      {row("clipboard", "9. ", "ENVIRONMENT VALIDATION",
          ...
          '<ul><li>Verify required tools</li><li>Check variables and configs</li><li>Validate dependencies</li><li>Fail fast if something is wrong</li></ul>' +
      ...
      {row("cube", "10. ", "WHERE SHELL SCRIPTING FITS",
          ...
          '<ul><li>Glue between tools</li><li>Automate small and big tasks</li><li>Provide flexibility</li><li>Save time and reduce errors</li><li>Core skill for every DevOps engineer</li></ul>')}
    """
...
def main():
    ...
    # Verify each output references the colorful icon library
    import re as _re
    print("\n--- Verification ---")
    for page_num, _ in pages:
        out_path = OUTPUT_DIR / f"recreated-page-{page_num:02d}.html"
        content = out_path.read_text(encoding="utf-8")
        # Strip CSS comments for accurate branding check
        content_no_css_comments = _re.sub(r'/\*.*?\*/', '', content, flags=_re.DOTALL)
        # Count colorful icons (viewBox="0 0 48 48")
        icon_count = content.count('viewBox="0 0 48 48"')
        # Verify canonical CSS inlined (check for distinctive marker)
        has_css = "CANONICAL NOTEBOOK TEMPLATE CSS" in content
        # Verify body class="multi-page"
        has_multi = 'class="multi-page"' in content
        # Verify NO VERIQTA branding in visible content (excluding CSS comments)
        no_veriqta = "VERIQTA" not in content_no_css_comments.upper()
        # Verify NO social footer
        no_social = "social-footer" not in content
        print(f"  Page {page_num}: {icon_count} colorful icons, css_inlined={has_css}, "
